...
--Give Gilberts Key
local bGilbert = debugGui:Button("Give Gilberts Key",
	function()
...
--Find HotSpots
local bHotSpots = mainGui:Button("Find HotSpots", 
 function()
	local ignoreF, destroyF = pcall(
		function()
			pcall(game.Workspace.Hotspots:Destroy())
		end
	)
	local createFolder = Instance.new("Folder", game.Workspace)
	createFolder.Name = "Hotspots"
	dprint("Cleared Existing Hotspots")
	for i, v in pairs (game.ReplicatedStorage.CloudClientResources.Communication.Custom.HotFishingAreas:GetChildren()) do
		local newHotspot = Instance.new("Part", game.Workspace.Hotspots)
		newHotspot.BrickColor = BrickColor.new("Hot Pink")
		newHotspot.CFrame = CFrame.new(v.Value)
		newHotspot.CanCollide = false
		newHotspot.Anchored = true
		newHotspot.Size = Vector3.new(0, 0, 0)
		dprint("Created Hotspot Part")

		local x = Instance.new('BillboardGui',newHotspot)
		x.AlwaysOnTop = true
		x.Size = UDim2.new(1, 0, 1, 0)
		local b = Instance.new('Frame', x)
		b.Size = UDim2.new(1, 0, 1, 0)
		x.Adornee = newHotspot
...
--Show Sharks
local bShowSharks = mainGui:Button("Show Sharks", 
	function()
		for i, v in pairs(game.Workspace:GetChildren()) do
...
--Kill Moby Wood
local prevLocation
local bKillMobyWood = mainGui:Button("Kill Moby Wood", 
	function()
		dprint("Kill Moby Wood")
		for i, v in pairs(game.Workspace:GetChildren()) do
			if v:FindFirstChild("Health") and v:FindFirstChild("IsSeaBoss") and v.Name == "WhaleBoss" then
				prevLocation = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
				if game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool") then
					if game.Players.LocalPlayer.Character.Humanoid.Sit then
						game.Players.LocalPlayer.Character.Humanoid.Sit = false
